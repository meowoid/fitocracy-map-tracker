...
class fmtSelenium(object):
  ...
  def fitocracy_login(self, username=None, password=None):
    ...
    log.info('Retrieving login_url... %s sec', elapsed())
  ...
  def getActivities(self, activity=None):
    ...
    log.info('Retrieving profile_url... %s sec', elapsed()) 
    self.driver.get(self.profile_url)
    time.sleep(2)
    fitEvents = self.driver.find_elements_by_class_name('stream_item')
    activities = {}
    log.info('Scraping profile events... %s sec', elapsed())
    for event in fitEvents:   
      k = event.get_attribute('data-ag-type')
      v = event.text
      activities.setdefault(k, []).append(v)    
    #return activities # dict with types as key and activity tuple as val
...
if __name__ == '__main__':
  ...
  fmtSel = fmtSelenium()
  if fmtSel.fitocracy_login(username, password): 
    print fmtSel.getActivities() 
  else: 
